...
Page({
  ...
  handleRecognitionResult: function (result) {
    ...
    if (this.data.article && this.data.article.content) {
      ...
      // 计算相似度
      const similarityResult = calculateTextSimilarity(originalText, recognitionText);
      const similarity = typeof similarityResult === 'object' ? similarityResult.similarity : similarityResult;
      ...
      let score = similarity < 80 ? similarity : Math.min(100, similarity * 1.1);
      score = Math.round(score);
      
      // 生成评价信息
      let scoreComment = '';
      if (score >= 90) {
        scoreComment = '完美背诵，记忆准确';
      } else if (score >= 80) {
        scoreComment = '背诵优秀，略有遗忘';
      } else if (score >= 70) {
        scoreComment = '背诵良好，有待提高';
      } else if (score >= 60) {
        scoreComment = '背诵尚可，需要记忆';
      } else {
        scoreComment = '需要多加练习';
      }
  ...
  handleScoreResult: function (result) {
    ...
    let score = Math.floor(result.score || 0);
  ...
  savePracticeData: function (score) {
    try {
      ...
      const practiceRecords = wx.getStorageSync('reciting_records') || {};
      ...
      wx.setStorageSync('reciting_records', practiceRecords);
      console.log('保存背诵数据成功');
    } catch (e) {
      console.error('保存背诵数据失败', e);
    }
  },
  ...
  playText: function() {
    ...
    speechService.textToSpeech(textToSpeak, speechRate)
      .then(filePath => {
        ...
        // 必须设置title，否则无法播放
        backgroundAudioManager.title = this.data.article.title || '背诵练习';
        // 设置播放源
  ...
  startRecording: function() {
    ...
    this.setData({
      ...
      scoreComment: '',
      recordingDuration: 0,
      isRecording: true // 设置录音状态为true，用于隐藏原文
    });
    
    // 开始录音评测
    speechService.startRecording(text)
      .then(() => {
        console.log('开始录音评测（背诵模式）');
   ...
   * 完成背诵练习，保存记录并返回
   */
  finishReciting: function() {
    // 保存本次练习记录
    const recordData = {
      articleId: this.data.article._id || this.data.article.id,
      title: this.data.article.title,
      score: this.data.score,
      duration: this.data.recordingDuration,
      date: new Date().getTime(),
      practiceType: APP_CONSTANTS.PRACTICE_TYPES.RECITE // 修正引用方式
    };
    ...
    try {
      ...
      // 保存记录，最多保存100条
      wx.setStorageSync('practice_records', records.slice(0, 100));
